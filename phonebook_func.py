...
def center_window(self,w,h): # pass in the tkinter frame (master) reference and the w and h
    # get user's screen height and width
    screen_width = self.master.winfo_screenwidth()
    screen_height = self.master.winfo_screenheight()
    # calculate x andy coordinates to paint the app centered on the user's screen
...
def ask_quit(self):
    if messagebox.askokcancel('Exit program', 'Okay to exit application?'):
        # this closes app
        self.master.destroy()
        os._exit(0) #frees up memory when program is closed
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname, col_lname, col_phone, con_email FROM tbl_phonebook WHERE col_fullname = (?)""", [value])
...
def onDelete(self):
    ...
    with conn:
        ...
        if count > 1:
            ...
            confirm = messagebox.showerror('Last Record Error','({}) is the last record in the database and cannot be deleted at this time. \n\nPlease add another record before deleting ({}).'.format(var_select,var_select))
    conn.close()
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            # the database, meaning there are no changes to updatae
